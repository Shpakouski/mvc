...
class Model
{
    ...
    public function __construct($id = null)
    {
        ...
        $this->instanceDb = DbDriver::getInstance();
    ...
    public function where($field, $operator = "", $value = "")
    {
        if (func_num_args() == 2) {
            ...
            $this->where[] = $field . " " . $operator . " " . $this->instanceDb->getInstanceDb()->real_escape_string($value);
        }
        ...
        if ($this->sql) {
            switch ($this->typeSql) {
                case 'select':
                    ...
                    if (!$result) {
                        throw new \Exception("Ошибка запроса" . $this->instanceDb->connect_errno . "|" . $this->instanceDb->connect_error);
                    }
